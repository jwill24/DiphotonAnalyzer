...
void analyze_efficiency()
{
  ...
  map<unsigned short,pair<double,double> > pot_fit_limits = { { 2, { 9., 14. } }, { 3, { 8., 14. } }, { 102, { 9., 13. }  }, { 103, { 8., 13. } } };
  ...
  for ( unsigned long long i = 0; i < num_entries; ++i ) {
    ...
    for ( unsigned int j = 0; j < num_proton_track; ++j ) {
      ...
      if ( is_ref_fill ) {
        ...
        h2_denom_xy[pid]->Fill( trk_x, trk_y, 1./num_entries );
      }
    }
  }
      ...
      { // plot the efficiencies
        ...
        if ( distrib == "x" || distrib == "xi" || distrib == "xi_optbins" ) {
          ...
          if ( distrib == "x" ) {
            ...
            switch ( p.first ) {
              case 2: range_min = 0.052; break;
              case 3: range_min = 0.047; break;
              case 102: range_min = 0.047; break;
              case 103: range_min = 0.030; break;
            }
            range_max = 0.15;
          }
  ...
  for ( const auto& p : pot_names ) {
    {
      ...
      ratio->GetZaxis()->SetRangeUser( 0.2, 1. );
