...
plotter()
{
  ...
  const string kin_regions[3] = { "nosel", "presel", "elastic" };
  typedef enum {
    nosel = 0,
    presel = 1,
    elastic = 2,
  ...
  for ( unsigned short i=0; i<num_samples; i++ ) {
    ...
    for ( unsigned short j=0; j<num_regions; j++ ) {
      ...
      h_mass[j][i] = new TH1D( Form( "h_%s_mass_%d", kin_regions[j].c_str(), i ), "Diphoton mass\\Events\\GeV", 40, 350., 1350. );
      ...
      h_diph_vtxz[j][i] = new TH1D( Form( "h_%s_diph_vtxz_%d", kin_regions[j].c_str(), i ), "Diphoton vertex z\\Events\\cm", 70, -35., 35. );
    ...
    else if ( s.type()==Sample::kSignal ) { type = 1; } //FIXME
  ...
  for ( unsigned short i=0; i<num_regions; i++ ) {
    plt.draw_multiplot( Form( "%s_diphoton_mult", kin_regions[i].c_str() ), hm_ndiph[i][0], hm_ndiph[i][1], true );
    plt.draw_multiplot( Form( "%s_diphoton_mass", kin_regions[i].c_str() ), hm_mass[i][0], hm_mass[i][1], false );
    plt.draw_multiplot( Form( "%s_diphoton_mass_logscale", kin_regions[i].c_str() ), hm_mass[i][0], hm_mass[i][1], true );
    plt.draw_multiplot( Form( "%s_diphoton_ptpair", kin_regions[i].c_str() ), hm_ptpair[i][0], hm_ptpair[i][1], false );
    plt.draw_multiplot( Form( "%s_diphoton_ptpair_logscale", kin_regions[i].c_str() ), hm_ptpair[i][0], hm_ptpair[i][1], true );
    plt.draw_multiplot( Form( "%s_diphoton_pt_singlepho", kin_regions[i].c_str() ), hm_pt[i][0], hm_pt[i][1], false );
    plt.draw_multiplot( Form( "%s_diphoton_pt_leadpho", kin_regions[i].c_str() ), hm_ptlead[i][0], hm_ptlead[i][1], false );
    plt.draw_multiplot( Form( "%s_diphoton_pt_subleadpho", kin_regions[i].c_str() ), hm_ptsublead[i][0], hm_ptsublead[i][1], false );
    plt.draw_multiplot( Form( "%s_diphoton_r9_singlepho", kin_regions[i].c_str() ), hm_r9[i][0], hm_r9[i][1], false );
    plt.draw_multiplot( Form( "%s_diphoton_r9_leadpho", kin_regions[i].c_str() ), hm_r9lead[i][0], hm_r9lead[i][1], false );
    plt.draw_multiplot( Form( "%s_diphoton_r9_subleadpho", kin_regions[i].c_str() ), hm_r9sublead[i][0], hm_r9sublead[i][1], false );
    plt.draw_multiplot( Form( "%s_diphoton_dphi", kin_regions[i].c_str() ), hm_dphi[i][0], hm_dphi[i][1] );
    plt.draw_multiplot( Form( "%s_diphoton_xip", kin_regions[i].c_str() ), hm_xip[i][0], hm_xip[i][1] );
    plt.draw_multiplot( Form( "%s_diphoton_xim", kin_regions[i].c_str() ), hm_xim[i][0], hm_xim[i][1] );
    plt.draw_multiplot( Form( "%s_diphoton_nvtx", kin_regions[i].c_str() ), hm_nvtx[i][0], hm_nvtx[i][1] );
    plt.draw_multiplot( Form( "%s_diphoton_vtx_z", kin_regions[i].c_str() ), hm_diph_vtxz[i][0], hm_diph_vtxz[i][1] );
  }
