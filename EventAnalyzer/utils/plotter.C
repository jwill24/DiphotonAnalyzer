...
plotter()
{
  ...
  Plotter::HistsMap hm_ndiph[num_regions][num_classes-2][3],
  ...
  for ( unsigned short i=0; i<num_samples; i++ ) {
    ...
    unsigned short sample_type = 0;
    if ( s.type()==Sample::kData ) { sample_type = 0; }
    else if ( s.type()==Sample::kBackground ) { sample_type = 1; }
    else if ( s.type()==Sample::kSignal ) { sample_type = 2; }


    for ( unsigned short k=0; k<num_classes-2; k++ ) {
      for ( unsigned short j=0; j<num_regions; j++ ) {
        const char* region = kin_regions[j].c_str();
        h_ndiph[j][k][i] = new TH1D( Form( "h_%s_%d_ndiph_%d", region, k, i ), "Number of diphoton candidates\\Events", 10, 0., 10. );
        h_xip[j][k][i] = new TH1D( Form( "h_%s_%d_xip_%d", region, k, i ), "Diphoton #xi^{+}\\Events", 50, 0., 0.5 );
        h_xim[j][k][i] = new TH1D( Form( "h_%s_%d_xim_%d", region, k, i ), "Diphoton #xi^{-}\\Events", 50, 0., 0.5 );
        h_diph_vtxz[j][k][i] = new TH1D( Form( "h_%s_%d_diph_vtxz_%d", region, k, i ), "Diphoton vertex z\\Events\\cm", 40, -20., 20. );
      }
      ...
      h_ptpair[presel][k][i] = new TH1D( Form( "h_%s_%d_ptpair_%d", kin_regions[presel].c_str(), k, i ), "Diphoton p_{T}\\Events\\GeV", 50, 0., 500. );
      ...
      h_r9[elastic][k][i] = new TH1D( Form( "h_%s_%d_r9_%d", kin_regions[elastic].c_str(), k, i ), "Single photon r_{9}\\Events\\?.2f", 30, 0.94, 1. );
...
//std::cout << "--> " << eff_pho1 << "\t" << eff_pho2 << std::endl;
    ...
    for ( unsigned short j=0; j<num_regions; j++ ) {
      for ( unsigned short k=0; k<num_classes-2; k++ ) {
        ...
        hm_ndiph[j][k][sample_type].emplace_back( s.name(), h_ndiph[j][k][i] );
        hm_mass[j][k][sample_type].emplace_back( s.name(), h_mass[j][k][i] );
        hm_ptpair[j][k][sample_type].emplace_back( s.name(), h_ptpair[j][k][i] );
        hm_met[j][k][sample_type].emplace_back( s.name(), h_met[j][k][i] );
        hm_ptlead[j][k][sample_type].emplace_back( s.name(), h_ptlead[j][k][i] );
        hm_ptsublead[j][k][sample_type].emplace_back( s.name(), h_ptsublead[j][k][i] )  ;
        hm_pt[j][k][sample_type].emplace_back( s.name(), h_pt[j][k][i] )  ;
        hm_r9lead[j][k][sample_type].emplace_back( s.name(), h_r9lead[j][k][i] );
        hm_r9sublead[j][k][sample_type].emplace_back( s.name(), h_r9sublead[j][k][i] );
        hm_r9[j][k][sample_type].emplace_back( s.name(), h_r9[j][k][i] );
        hm_dphi[j][k][sample_type].emplace_back( s.name(), h_dphi[j][k][i] );
        hm_xip[j][k][sample_type].emplace_back( s.name(), h_xip[j][k][i] );
        hm_xim[j][k][sample_type].emplace_back( s.name(), h_xim[j][k][i] );
        hm_diph_vtxz[j][k][sample_type].emplace_back( s.name(), h_diph_vtxz[j][k][i] );
      }
//cout << "moh " << j << "\t" << i << endl;
      h_nvtx[j][i]->Scale( sample_weight );
//cout << "meh " << j << "\t" << i << " >> " << sample_type << endl;
      //hm_nvtx[j][type].push_back( make_pair( s.name(), h_nvtx[j][i] ) );
      //if ( h_nvtx[j][i] ) hm_nvtx[j][sample_type].emplace_back( s.name(), h_nvtx[j][i] );
      //hm_nvtx[j][type].emplace_back( s.name(), (TH1D*)h_nvtx[j][i]->Clone() );
//cout << "muh " << j << "\t" << i << endl;
    }
...
std::cout << "event finished" << std::endl;
  ...
  for ( unsigned short i=0; i<num_regions; i++ ) {
    for ( unsigned short j=0; j<num_classes-2; j++ ) {
      plt.draw_multiplot( Form( "%s_diphoton_mult_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_ndiph[i][j][0], hm_ndiph[i][j][1], hm_ndiph[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ), true );
      plt.draw_multiplot( Form( "%s_diphoton_mass_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_mass[i][j][0], hm_mass[i][j][1], hm_mass[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ), false );
      plt.draw_multiplot( Form( "%s_diphoton_mass_logscale_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_mass[i][j][0], hm_mass[i][j][1], hm_mass[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ), true );
      plt.draw_multiplot( Form( "%s_diphoton_ptpair_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_ptpair[i][j][0], hm_ptpair[i][j][1], hm_ptpair[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ), false );
      plt.draw_multiplot( Form( "%s_diphoton_ptpair_logscale_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_ptpair[i][j][0], hm_ptpair[i][j][1], hm_ptpair[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ), true );
      plt.draw_multiplot( Form( "%s_diphoton_met_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_met[i][j][0], hm_met[i][j][1], hm_met[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_pt_singlepho_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_pt[i][j][0], hm_pt[i][j][1], hm_pt[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_pt_leadpho_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_ptlead[i][j][0], hm_ptlead[i][j][1], hm_ptlead[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_pt_subleadpho_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_ptsublead[i][j][0], hm_ptsublead[i][j][1], hm_ptsublead[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_r9_singlepho_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_r9[i][j][0], hm_r9[i][j][1], hm_r9[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_r9_leadpho_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_r9lead[i][j][0], hm_r9lead[i][j][1], hm_r9lead[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_r9_subleadpho_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_r9sublead[i][j][0], hm_r9sublead[i][j][1], hm_r9sublead[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_dphi_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_dphi[i][j][0], hm_dphi[i][j][1], hm_dphi[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_xip_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_xip[i][j][0], hm_xip[i][j][1], hm_xip[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_xim_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_xim[i][j][0], hm_xim[i][j][1], hm_xim[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
      plt.draw_multiplot( Form( "%s_diphoton_vtx_z_%s", kin_regions[i].c_str(), classes[j].c_str() ), hm_diph_vtxz[i][j][0], hm_diph_vtxz[i][j][1], hm_diph_vtxz[i][j][2], Form( "#font[62]{%s}", classes[j].c_str() ) );
    }
